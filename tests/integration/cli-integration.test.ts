...
describe('CLI Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    const mockWrite = jest.fn().mockImplementation((filePath: string, content: string) => {
  ...
  describe('Scan Command', () => {
    it('should scan Express.js project successfully', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('✅ Express.js project detected'));
      expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Dependencies:'));
      expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('express:'));
...
it('should generate Docker configuration in interactive mode', async () => {
  ...
  describe('Analyze Command', () => {
    ...
    it('should detect and display services in human-readable format', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Environment Configuration'));
      expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('MongoDB'));
      expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('Redis'));
    });
  });
  ...
  try {
    await cli.parseAsync(['node', 'test', 'express', 'generate', '--port', 'invalid']);
  } catch (error) {
  ...
  const promptMock = (inquirer.prompt as unknown as jest.Mock<Promise<Answers>>);
  ...
  expect(mockConsoleError.mock.calls[0][1]).toBe('Invalid configuration file');
});
  });
});